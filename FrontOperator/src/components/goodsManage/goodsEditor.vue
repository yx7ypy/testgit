				...
				<div class="con-box">
					<el-form ref="subData" :rules="rules" label-position="right" :model="subData" label-width="150px">
						<el-form-item label="商品编号：" prop="productNo">
							<el-input v-model="subData.productCode" placeholder="自动生成仅供参考" :disabled="true"></el-input>
						</el-form-item>
						...
						<el-form-item label="国际条形：" prop="barCode">
							<el-input v-model="subData.barCode" placeholder="请输入"></el-input>
						</el-form-item>
						...
						<el-form-item label="预估进价：" prop="purchasePrice">
							<el-input v-model="subData.purchasePrice" placeholder="请输入" @change="changePurchasePrice">
								<template slot="append">元</template>
							</el-input>
						</el-form-item>
						<el-form-item label="毛利率：" prop="am">
						...
						<el-form-item label="商品状态：">
							<div style="float: left;">
								<el-radio class="radio" v-model="subData.isDelete" label="0">启用中</el-radio>
								<el-radio class="radio" v-model="subData.isDelete" label="1">未启用</el-radio>
							</div>
						</el-form-item>
						<el-form-item label="图片：">
							<image-list-upload-editor :imageUrl="imageUrl" @increment="changeScucess"></image-list-upload-editor>
						</el-form-item>
					</el-form>
					<!--操作-->
					<div class="oper-box">
...
<script>
	...
	import imageListUploadEditor from '@/components/fengModule/imageListUploadEditor.vue';
	export default {
		data() {
			// 不含小数点
			...
			return {
				rules: {
					...
					alarmBefore: [
						{ required: true, message: '效期提醒不能为空', trigger: 'blur' },
					...
					purchasePrice: [
						{ required: true, message: '预估进价不能为空', trigger: 'blur' },
		...
		created() {
			...
			this.$api.apiCommunicationGet('/product/getById/' + _id, res => {
				...
				this.subData.am = ((this.subData.defaultPrice - this.subData.purchasePrice) / this.subData.defaultPrice * 100).toFixed(2);
				this.subData.isDelete = (this.subData.isDelete).toString();
				this.subData.expiration = (this.subData.expiration).toString(); // 保质期
				this.subData.alarmBefore = (this.subData.alarmBefore).toString();	// 效期
				this.subData.defaultPrice = (this.subData.defaultPrice).toString();	// 默认售价
				this.subData.purchasePrice = (this.subData.purchasePrice).toString();	//进价
				let _imageUrl = [];
				for(let item of res.data.productImages) {
					_imageUrl.push({ name: item.url, url: this.imageHost + item.url })
				}
		...
		methods: {
			...
			goHold(subData) {
				this.$refs[subData].validate((valid) => {
					if(valid){
						this.$api.apiCommunication('/product/update', this.subData, res => {
							if(res.code != 2000) {
								this.$alert(`保存失败，服务器返回信息：${res.msg}`, '系统通知', { confirmButtonText: '确定', type: 'error' })
								return false
							}
